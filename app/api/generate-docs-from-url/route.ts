...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const processingPromise = async () => {
      ...
      for (let i = 0; i < Math.min(concurrentLimit, relevantFiles.length); i++) {
        const startIndex = i;
        promises.push(processFileBatch(relevantFiles, startIndex, concurrentLimit));
      }
      
      async function processFileBatch(files: string[], startIndex: number, step: number): Promise<string> {
        let fileContent = "";
...
\n\n${allFileContents}\n\n`;
    ...
    if (errorMessage.includes('timeout')) {
      return new NextResponse(JSON.stringify({ 
        error: "The request timed out. Please try a smaller repository or more specific filtering options.", 
        isTimeout: true 
      }), { status: 504 });
    }
